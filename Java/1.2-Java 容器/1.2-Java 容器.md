...
> 相比容器特殊性：
> * 效率最高、随机访问,长度固定
> * 保存具体类型的**对象引用**序列（容器使用泛型）
> * 可以直接保存基本类型（容器使用自动包装类型）

1. 数组标识符（栈中）是数组对象（堆中）的引用
2. 数组对象保存指向其他对象的引用
3. 初始化
```java
/* 创建一维数组 */
Object[] a; // notInitialized
Object[] b = new Object[5];
Object[] c = {new Object(), new Object()}; // 聚集初始化，仅能在定义时使用
Object[] d = new Object[]{new Object(), new Object()} // 动态聚集初始化
d = b; // 长度不影响数组类型

/* 创建二维数组 */
Object[][] b = new Object[5][6];
Object[][] c = {{new Object()}
                 new Object(), new Object()};// 数组每个向量可以有不同的长度，成为粗糙数组
Object[][] b = new Object[][]{{new Object()}
                 new Object(), new Object()};

```
4. 返回一个数组
    * 实际数组对象在堆中创建，实际返回一个数组引用（数据标识符赋值）
    * 实际数组对象的生命周期由GC机制管理
5. 相关接口
```java
Arrays.equals();
...
Arrays.toString();
Arrays.hashCode();
System.arrayCopy(src, srcPos, dest, destPos, length); // 高效率复制数组
...
```java
PriorityQueue
(Array/Linked/Priority)BlockingQueue
EnumSet
EnumMap
```
![123](../..//pic/java-container-1-2.jpg)
...
- TreeSet：类似cpp中set，基于红黑树实现，须实现`Comparable<>`接口，支持有序性操作，例如根据一个范围查找元素的操作。但是查找效率不如 HashSet，HashSet 查找的时间复杂度为 O(1)，TreeSet 则为 O(logN)。Java中基于TreeMap实现。

- HashSet：类似cpp中unorderedMap，基于哈希表实现，须实现`hashCode()`接口，支持快速查找，但不支持有序性操作。并且失去了元素的插入顺序信息，也就是说使用 Iterator 遍历 HashSet 得到的结果是不确定的。Java中基于HashMap实现。

- LinkedHashSet：须实现`hashCode()`接口，具有 HashSet 的查找效率，并且内部使用双向链表维护元素的插入顺序。

### 2. List

- ArrayList：基于动态数组实现，支持随机访问。每次扩容为之前的1.5倍。
...
- TreeMap：基于红黑树实现，有序，查找效率为O(logn)

- HashMap：基于哈希表（数组+链表）实现，无序。 HashMap的位桶数组，初始大小为16。实际使用时，显然大小是可变的。如果位桶数组中的元素达到(**0.75*数组** length)， 就重新调整数组大小变为**原来2倍**大小。当对应链表长度大于8时，链表就转换为红黑树
