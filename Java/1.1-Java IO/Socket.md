...
应用进程使用 sigaction 系统调用，内核立即返回，应用进程可以继续执行，也就是说等待数据阶段应用进程是非阻塞的。内核在数据准备好时向应用进程发送 SIGIO 信号，应用进程收到之后在信号处理程序中调用 recvfrom 将数据从内核复制到应用进程中。
...
- 同步 I/O：将数据从内核缓冲区复制到应用进程缓冲区的阶段（第二阶段），即调用`recvfrom`后应用进程会阻塞。
- 异步 I/O：第二阶段应用进程不会阻塞。
...
select 和 poll 速度都比较慢，每次调用都需要将**全部描述符**从应用进程缓冲区**复制到内核缓冲区**。
...
1. `epoll_create` 创建一个 epoll 的句柄，size 用来告诉内核这个监听的数目一共有多大（hint 而已，实际不起作用）。需要注意的是，当创建好 epoll 句柄后，它就是会占用一个 fd 值，之后返回 fd。
2. `epoll_ctl` 用于向内核注册新的描述符，也可以改变某个文件描述符的状态。已注册的描述符在内核中会被维护在一棵红黑树上，通过回调函数，内核会将 I/O 准备好的描述符加入到一个链表中管理，进程调用 epoll_wait() 便可以得到事件完成的描述符。
3. `epoll_wait` 会使调用它的进程进入睡眠（timeout为0时除外），如果有监听的事件产生，该进程就被唤醒，同时将事件从内核里面拷贝到用户空间返回给该进程。

从上面的描述可以看出，epoll 只需要将描述符从进程缓冲区向内核缓冲区**拷贝一次**，并且进程**不需要通过轮询**来获得事件完成的描述符。
...
int main(int argc, char* argv[])
{
    ...
    epfd=epoll_create(5);  //保存 epoll 的数据

    struct sockaddr_in clientaddr;
    struct sockaddr_in serveraddr;
    listen_fd = socket(AF_INET, SOCK_STREAM, 0);  //返回监测5000端口的 Scoket fd
    //设置与要处理的事件相关的文件描述符
    ...
    for ( ; ; ) 
    {
        ...
        for (int i = 0; i < nfds; ++i)
        {
            ...
            {            
                ...
                //修改socket_fd上要处理的事件为EPOLLOUT,暂时屏蔽此功能
                // epoll_ctl(epfd,EPOLL_CTL_MOD,socket_fd,&ev);
            }
            else if (events[i].events&EPOLLOUT) // 如果有数据发送
            {
                socket_fd = events[i].data.fd;
                char* send = "receive succ, line out\n";
                write(socket_fd, send, 24);
                //设置用于读操作的文件描述符
                ev.data.fd = socket_fd;
                //设置用于注测的读操作事件
                ev.events = EPOLLIN|EPOLLET;
