...
![Java技术体系
](../../pic/java-vitual-machine-1-1.png)
...
* 每个 Java 方法在执行的同时会创建一个栈帧用于存储局部变量表、操作数栈、常量池引用、动态连接、方法出口等信息。
...
- HotSpot不使用动态扩展，因此仅在线程申请空间失败时抛出OOM异常
...
注意：HotSpot虚拟机不区分虚拟机栈与本地方法栈，因此`-Xoss`参数无效
...
用于存放已被加载的类信息（访问修饰符、字段描述、方法描述）、常量、静态变量、即时编译器编译后的代码等数据。
...
方法区是一个 JVM 规范，永久代与元空间都是其一种实现方式。在 JDK 1.8 之后，原来永久代的数据被分到了堆和元空间中。元**空间存储类的元信息，静态变量和常量池等放入堆中。**

## 运行时常量池

> 运行时常量池是方法区的一部分。JDK7起常量池移至**堆**中
...
|区域|异常|条件|备注|虚拟机参数|
|:-:|:-:|:-:|:-:|:-:|
|堆|OOM|不断创建对象|常量区划归在堆内|`-Xms -Xmx -XX:+HeapDumpOnOutOfMemoryError`|
|虚拟机栈、本地方法栈|SOE|不断递归、大量定义本地变量|多线程情况可能OOM|线程栈大小`-Xss`、`-Xoss`|
|方法区|OOM|创建大量类型|元空间、永久代|`-XX:MaxMetaspaceSize、MetaspaceSize、MinMetaspaceSize`|
|直接内存|OOM|||`-XX:MaxDirectMemorySize=10M`|

## HotSpot 中的堆对象
### 对象创建
* 语言层面：普通对象通过new关键字创建
* 虚拟机层面：  
  1. 执行new指令，在常量池中定位类的符号引用
  2. 检查是否加载，否则加载类  
  3. 分配内存
     * 指针碰撞方式（带空间压缩整理Compact的收集器，Serial、ParNew等）
     * 空闲列表方式
     * 并发安全性：CAS或TLAB（本地线程分配缓存）
  4. 初始化
     * 对象头：运行时数据、类元数据
     * 对象实例：**初始化为0**，TLAB中可提前设置
  5. 执行invokespecial指令，调用构造函数
  ![](../../pic/java-vitual-machine-2-1.png)



### 内存布局
* **对象头**
  1. Mark word：存储运行时数据，32b或64b  
  2. 类型指针：指向类型元数据(optional)，若为数组，还必须记录数组长度
  
* **对象实例数据**：存储字段，父类在子类之前，默认分配顺序为:

  > ​                **long/double、int、short/char、bytes/boolean、reference**

* **对齐填充**：填充到8字节的整数倍（HotSpot的**自动内存管理系统**要求对象其实地址为8的整数倍）
|状态|标志位|Mark word 存储内容|
|:-:|:-:|:-:|
|未锁定|01|hash code(25)+分代年龄(4)+偏向锁(1b固定为0)+锁标志(2)|
|轻量级锁定|00|指向锁记录的指针|
|重量级锁定|10|指向重量级锁的指针|
|GC标记|11|空|
...
### 对象访问
* 句柄方式：Shenandoah收集器，GC时不需修改引用本身
![](../../pic/java-vitual-machine-2-2.jpg)
* 直接指针方式：HotSpot默认方式，少一次间接访问
![](../../pic/java-vitual-machine-2-3.jpg)
...
被弱引用关联的对象一定会被回收，也就是说它只能存活到下一次垃圾回收发生之前。当垃圾收集器开始工作，无论当前内存是否足够，都会回收掉只 被弱引用关联的对象。
...
现在的商业虚拟机采用分代收集算法，它根据对象存活周期将内存划分为几块，不同块采用适当的收集算法。其理论依据为以下假说：
...
* 老年代收集（Major/Old GC）：仅在CMS收集器中存在

* 整堆收集（Full GC）：堆+方法区

* 混合收集（Mixed GC）：G1收集器面向堆内存任何部分组成的回收集
...
以上是 HotSpot 虚拟机中的 7 个垃圾收集器，连线表示垃圾收集器可以配合使用，Serial与CMS、ParNew与Serial Old已废弃。

- 单线程与多线程：单线程指的是垃圾收集器只使用一个线程，而多线程使用多个线程，并行执行；
...
- 维护记忆集、写屏障和原始快照复杂，占用内存为堆的20%
  
### 8.低延迟收集器——Shenandoah、ZGC
